...
double time_elapse(void){
	...
	return  (double)(nEndTime.QuadPart - m_nBeginTime.QuadPart) * 1000 / m_nFreq.QuadPart;
}
...
void get_global_work_size3d(size_t x, size_t y, size_t z,  size_t* global_work_size3d)
{
 ...
 cl_program createProgramWithSource(cl_context context, cl_device_id * devices, size_t device_num, const char** kernel_name, size_t kernel_num, const char* options)
  {
	  cl_int errNum;
	  cl_program program;
	  //size_t* kernel_size = (size_t *) malloc(kernel_num * sizeof(size_t));
	  ...
	  if(!check_errNum(errNum))
	  {
		  ...
		  for(int i=0; i < device_num; ++i)
		  {
			  ...
			  char* buildLog = (char*)malloc(buildLogSize * sizeof(char));
			  clGetProgramBuildInfo(program, devices[i], CL_PROGRAM_BUILD_LOG, sizeof(char) * buildLogSize, buildLog, NULL);
			  size_t device_name_length = strlen(buffer);
			  sprintf(buffer + device_name_length, " ,构建程序日志: " );
			  printf("%s(%d)\n%s\n\n", buffer,(int)buildLogSize,  buildLog);
			  fprintf(fp, "%s(%d)\n%s\n\n", buffer,(int)buildLogSize,  buildLog);
			  free(buildLog);
		  }
		  fclose(fp);
...
bool saveProgramBinary(cl_program program)
{
	int device_num = 0;
	cl_int errNum;
	...
	if(!check_errNum(errNum))
		return false;
	cl_device_id *devices = (cl_device_id*)malloc(device_num * sizeof(cl_device_id));
	errNum = clGetProgramInfo(program, CL_PROGRAM_DEVICES,
	                              sizeof(cl_device_id) * device_num, devices, NULL);
	if(!check_errNum(errNum))
	{
		free(devices);
		return false;
	}
	char** binaryName = (char**)malloc(device_num * sizeof(char*));
	for(int i = 0; i < device_num; ++i)
	{
		binaryName[i] = (char*)malloc(100 * sizeof(char));
		clGetDeviceInfo(devices[i], CL_DEVICE_NAME, 100, binaryName[i], NULL);
		strcat(binaryName[i], "_Binary.bin");
	}

	size_t *programBinarySizes = (size_t*)malloc(device_num * sizeof(size_t));
	errNum = clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES,
	                              sizeof(size_t) * device_num,
	...
	uchar **programBinary = (uchar**)malloc(device_num * sizeof(uchar*));
	for(int i = 0; i < device_num; ++i)
		programBinary[i] = (uchar*)malloc(programBinarySizes[i] * sizeof(char));
	errNum = clGetProgramInfo(program, CL_PROGRAM_BINARIES, sizeof(uchar*) * device_num,
	                              programBinary, NULL);
	if(!check_errNum(errNum))
	{
		free(devices);
	...
	for (int i = 0; i < device_num; ++i)
	{
		...
		fwrite(programBinary[i], sizeof(uchar), programBinarySizes[i], fp);
		fclose(fp);
	}
...
cl_mem cl_pull_array(cl_command_queue command, void* host_data, cl_mem device_data, size_t size)
{
 ...
 cl_context CreateContext(cl_device_id *device)
 {
	 ...
	 if (errNum != CL_SUCCESS)
	 {
		 printf("Failed to create any OpenCL platforms.");
		 free(platform);
		 return NULL;
	 }
	 size_t namesize;
	 for (cl_uint i = 0; i < numPlatforms; ++i) {
		 errNum = clGetPlatformInfo(platform[i], CL_PLATFORM_NAME, 0, NULL, &namesize);
		 if (errNum != CL_SUCCESS)
		 {
			 printf("Failed to query OpenCL platform namesize.");
			 free(platform);
			 return NULL;
		 }
		 char* name = (char*)malloc(namesize * sizeof(char));
		 errNum = clGetPlatformInfo(platform[i], CL_PLATFORM_NAME, namesize, name, NULL);
		 if (errNum != CL_SUCCESS)
		 {
			 printf("Failed to query OpenCL platform name.");
			 free(platform);
			 return NULL;
		 }
		 //if (NULL != strstr(name, "Intel(R) FPGA")) {
		 if (NULL != strstr(name, "AMD")) {
     ...
     //errNum = clGetDeviceIDs(FPGA_platform, CL_DEVICE_TYPE_ACCELERATOR, 1, device, NULL);
	 errNum = clGetDeviceIDs(FPGA_platform, CL_DEVICE_TYPE_GPU, 1, device, NULL);
     if (errNum != CL_SUCCESS)
     {
        //  printf( "There is NO FPGA" );
