...
cl_int fill_on_ocl(cl_command_queue command, int N, float ALPHA, cl_mem  X, int INCX)
{
	...
	//printf("lllllllllllllllllllllllllllllllllllll\n");
...
{
	cl_int errNum;
	//cl_program program = Program[UTILS_PROGRAM];
	cl_program program = Program;
...
//__kernel void upsample_kernel(unsigned long N, __global float *x, int w, int h, int c, int batch, int stride, int forward, float scale,  __global float *out)

//	   upsample_ocl(commandQueue, state.input_ocl, l.w, l.h, l.c, l.batch, l.stride, 1, l.scale, l.output_ocl);
