...
#ifdef OCL
    l.forward_ocl = forward_cost_layer_ocl;
    //l.backward_gpu = backward_cost_layer_gpu;

    l.delta_ocl = cl_make_array(context, l.output, inputs * batch * sizeof(float));
    l.output_ocl = cl_make_array(context, l.delta, inputs * batch * sizeof(float));
#endif
    return l;
}
...
#ifdef OCL
void forward_cost_layer_ocl(cost_layer l, network_state state)
{

	cl_int errNum;
    if (!state.truth_ocl) return;
    errNum = l2_ocl(commandQueue, l.batch * l.inputs, state.input_ocl, state.truth_ocl, l.delta_ocl, l.output_ocl);
    if(!check_errNum(errNum))
    {
    	fprintf(stderr,"forward_cost_layer_ocl中l2_ocl执行失败, 退出程序\n");
    	exit(1);
    }
    errNum = clEnqueueReadBuffer(commandQueue, l.output_ocl, CL_TRUE, 0, l.batch * l.inputs * sizeof(float), l.output, 0, NULL, NULL);
    if(!check_errNum(errNum))
    {
    	fprintf(stderr,"forward_cost_layer_ocl中读取buffer执行失败, 退出程序\n");
    	exit(1);
    }
    l.cost[0] = sum_array(l.output, l.batch*l.inputs);
}
