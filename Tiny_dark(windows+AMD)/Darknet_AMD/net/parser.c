...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[mp2]") == 0) return MP2;
    return BLANK;
}
...
#elif defined OCL
        if(gpu_index >= 0)
        	net.workspace_ocl = cl_make_array(context, NULL, workspace_size);
        else
        	net.workspace = calloc(1, workspace_size);
#else
        net.workspace = calloc(1, workspace_size);
...
#ifdef OCL
    if(gpu_index >= 0)
        push_convolutional_layer_ocl(l);
    cl_int errNum;
    float tmp[20];
    errNum = clEnqueueReadBuffer(commandQueue, l.rolling_mean_ocl, CL_TRUE, 0, 20 * sizeof(float), tmp, 0, NULL, NULL);
	/*printf("*************file: %s, line: %d, function: %s\n", __FILE__, __LINE__, __FUNCDNAME__);
	for (int ii = 0; ii < 40; ii++)
		printf("%f, ", tmp[ii]);
	printf("\n");*/
    errNum = clEnqueueReadBuffer(commandQueue, l.rolling_variance_ocl, CL_TRUE, 0, 20 * sizeof(float), tmp, 0, NULL, NULL);
	/*printf("*************file: %s, line: %d, function: %s\n", __FILE__, __LINE__, __FUNCDNAME__);
	for (int ii = 0; ii < 40; ii++)
		printf("%f, ", tmp[ii]);
	printf("\n");*/
...
void load_weights_upto(network *net, char *filename, int cutoff)
{
	...
	//printf("*************file: %s, line: %d, function: %s\n", __FILE__, __LINE__, __FUNCDNAME__);
	//printf("major:%d, minor: %d, revision: %d, net->seen: %d\n", major, minor, revision, net->seen[0]);
