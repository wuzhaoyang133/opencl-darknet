...
//        float *predictions = network_predict(net, X);
//		//for (int kk = 0; kk < 20; kk++)
//		//	printf("predictions[%d]:%f\n", kk, predictions[kk]);
//        //if(net.hierarchy) hierarchy_predictions(predictions, net.outputs, net.hierarchy, 0);
//		//printf("net->hierarchy?NULL:%d\n", net.hierarchy==NULL);
...
//	char buff[256];
//	char *input = buff;
//	char **filelist = get_labels("data/Imagenet2012_valid.list");
//	int count = 0;
//	// int size = net.w;
//	//while (1) {
...
//	}
//}
// classifier predict cfg/imagenet1k.data cfg/darknet19.cfg darknet19.weights data/Imagenet2012_valid.list
void predict_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top)
{
	printf("in predict classfier\n");
	printf("%s\n%s\n%s\n%s\n",datacfg,cfgfile,weightfile,filename);
	network net = parse_network_cfg(cfgfile);
	if (weightfile) {
		load_weights(&net, weightfile);
	}
...
void test_classifier(char *datacfg, char *cfgfile, char *weightfile, int target_layer)
{
	int yanse[20][3] = { { 1,208,0 },{ 123,255,0 },{ 149,156,96 },{ 0,135,0 },{ 1,78,0 },{ 160,80,43 },{ 0,213,213 },{ 255,255,255 },{ 216,191,216 },{ 248,0,0 },{ 195,195,195 },{ 117,149,142 },{ 176,0,0 },{ 78,0,0 },{ 146,73,47 },{ 251,251,0 },{ 230,202,19 },{ 222,0,206 },{ 0,0,227 },{ 179,198,223 } };   //定义颜色变量
	int curr = 0;   //定义计算器，计算已经处理了多少张图片
	int w, h, c, channels = 3;
	char buff[256] = "test_fact";   //保存最终结果图片的路径
	image img0 = load_image_color("haha.png", 0, 0);   //读入一张黑色三通道图像以生成测试结果图																				
	fflush(stdout);   //清理标准输入流，把多余的未被保存的数据丢掉
	network net = parse_network_cfg(cfgfile);   //加载网络ciafr_small.cfg
	if (weightfile) {
		load_weights(&net, weightfile);   //加载训练好的模型权重文件
	}
	srand(time(0));   //给算法一个启动种子，也就是算法的随机种子数，有这个数以后才可以产生随机数
	list *options = read_data_cfg(datacfg);   //加载文件cifar.data
	char *test_list = option_find_str(options, "test", "data\\cifar\\zz.txt");   //加载存放测试图像路径的文件
	int classes = option_find_int(options, "classes", 2);   //classes = 20
	list *plist = get_paths(test_list);
	char **paths = (char **)list_to_array(plist);   //图像的路径列表
	int m = plist->size;   // 测试图像个数
	free_list(plist);
	clock_t time;
	data val, buffer;
	load_args args = { 0 };
	...
	args.m = 0;
	args.labels = 0;
	args.d = &buffer;   //指向测试图片信息
	args.type = OLD_CLASSIFICATION_DATA;
	pthread_t load_thread = load_data_in_thread(args);   //创建线程，并且此函数实现了将测试图片信息读入到网络数据args.d.X（image)和args.d.Y(gt)中
	clock_t c_start, c_end, cc;
	c_start = clock();
	FILE *fpWrite = fopen("result_gpu.txt", "w");
	for (curr = net.batch; curr < m; curr += net.batch) {
		//time=clock();
		pthread_join(load_thread, 0);   //函数pthread_join用来等待一个线程的结束,第一个参数为被等待的线程标识符，第二个参数为一个用户定义的指针，它可以用来存储被等待线程的返回值
		val = buffer;
		if (curr < m) {
			args.paths = paths + curr;
			if (curr + net.batch > m) args.n = m - curr;
		...
		for (i = 0; i < pred.rows; ++i) {
			...
			col = atoi(str);
			//printf("--------------------------------------------col: %d\n", col);
			img0.data[0 * 1202 * 4768 + (row - 1) * 4768 + col - 1] = yanse[xuhao][0] / 255.;
			img0.data[1 * 1202 * 4768 + (row - 1) * 4768 + col - 1] = yanse[xuhao][1] / 255.;
			img0.data[2 * 1202 * 4768 + (row - 1) * 4768 + col - 1] = yanse[xuhao][2] / 255.;
		}
		free_matrix(pred);
		fprintf(stderr, "%lf seconds, %d images, %d total\n", sec(clock() - time), val.X.rows, curr);
		//free_data(val);
