...
void forward_batchnorm_layer_ocl(layer l, network_state state)
{
    ...
    if (state.train)
    {
      ...
      /*
       float tmp[20];
        errNum = clEnqueueReadBuffer(commandQueue, l.mean_ocl, CL_TRUE, 0, 20 * sizeof(float), tmp, 0, NULL, NULL);
        printf("l.mean_col is :");
        for(int i = 0;i < 20; ++i){ printf("%f ", tmp[i]); } printf("\n");
        */
