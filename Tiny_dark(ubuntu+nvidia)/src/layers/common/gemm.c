...
cl_int gemm_nt_ocl(cl_command_queue command, int M, int N, int K, float ALPHA, cl_mem A, size_t A_offset, cl_mem B, size_t B_offset, float BETA, cl_mem C, size_t C_offset)
{
	...
	if(!check_errNum(errNum))
	{
		...
		fprintf(stderr,"gemm_nt_ocl中执行矩阵转置失败\n");
		return errNum;
	}
        ...
        cl_mem C,  size_t C_offset)
{
	...
	// printf("chech diff from cpu\n");
	// float* a = (float*)malloc(sizeof(float) * M*N);
	// errNum = clEnqueueReadBuffer(commandQueue, A, CL_TRUE, 0, M*N * sizeof(float), a, 0, NULL, NULL);
	// float* b = (float*)malloc(sizeof(float) * K*N);
	// errNum = clEnqueueReadBuffer(commandQueue, B, CL_TRUE, 0, N*K * sizeof(float), b, 0, NULL, NULL);
	// float* c1 = (float*)malloc(sizeof(float) * M*K);
	// for (int i = 0; i < M*K; ++i)
	// {
	// 	c1[i]=0;
	// }
	// float* c2 = (float*)malloc(sizeof(float) * M*K);

	// gemm(TA, TB, M, K, N, 1.0, a, N, b, (TB==0)?K:N, 1 ,c1, K);
	// errNum = clEnqueueReadBuffer(commandQueue, C, CL_TRUE, 0, M*K * sizeof(float), c2, 0, NULL, NULL);
	// for (int i = 0; i < M*K; ++i)
	// {
	// 	// printf("%.2f\n", c1[i]);
	// 	if(abs(c1[i]-c2[i])>0.0001){
	// 		printf("wrong %d %.2f %.2f\n", i,c1[i],c2[i]);
	// 	}
	// }
	// errNum = clEnqueueWriteBuffer(commandQueue, C, CL_TRUE, 0, M*K * sizeof(float), c1, 0, NULL, NULL);
	return errNum;
}
#endif
